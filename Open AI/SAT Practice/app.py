...
def explain_ext():
    ...
    if not  user_collection.find_one({'_id': user_id, 'conversation._id': question_id}):
        create_new_question(user_id, question_id)
    
    update_chat(user_id, question_id, get_explanation_prompt(html))
...
def chat():
    ...
    chat = get_chat(user_id, question_id)
    chat.append({'role': 'user', 'content': message})
    update_chat(user_id, question_id, chat)    
